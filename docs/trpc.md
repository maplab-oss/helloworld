...
The `@maplab-oss/helloworld-trpc` package has two entry points:

- **`@maplab-oss/helloworld-trpc`** - Client-safe exports (types and schemas only). Use this in frontend code.
- **`@maplab-oss/helloworld-trpc/server`** - Server-only exports (includes `appRouter`). Use this in backend/worker code only.
